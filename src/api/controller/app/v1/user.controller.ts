...
function isAlbumFiles(f: unknown): f is AlbumFiles {
  return (
    typeof f === "object" &&
    f !== null &&
    "album" in f &&
    (f as unknown as AlbumFiles).album !== null
  );
}

const toMediaFile = (file: UploadedFile): MediaFile => ({
...
export const uploadMedia = async (
  ...
  try {
    if (!isAlbumFiles(req.files)) {
      return errorRes(res, "Album file is required.");
    }
...
export const uploadSocketMedia = async (
  ...
  try {
    ...
    type ReqWithFiles = Request & { files: FileFields & FileArray };

    const { album_type, ln } = req.body;
    const { album, thumbnail } = (req as ReqWithFiles).files;
    ...
    for (let i = 0; i < albumType.length; i++) {
      ...
      if (res_upload_file.status) {
        if (album_type_i === "image") {
          ...
          const thumbnail_image_path = null as null;
